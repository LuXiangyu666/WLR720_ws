...
let VanjeePacket = require('./VanjeePacket.js');
...
class VanjeeScan {
  ...
  static serialize(obj, buffer, bufferOffset) {
    ...
    obj.packets.forEach((val) => {
      bufferOffset = VanjeePacket.serialize(val, buffer, bufferOffset);
    });
    return bufferOffset;
  }
  ...
  static deserialize(buffer, bufferOffset=[0]) {
    ...
    for (let i = 0; i < len; ++i) {
      data.packets[i] = VanjeePacket.deserialize(buffer, bufferOffset)
    }
    return data;
  }
  ...
  static getMessageSize(object) {
    ...
    length += 1608 * object.packets.length;
    return length + 4;
  }
  ...
  static Resolve(msg) {
    ...
    const resolved = new VanjeeScan(null);
    ...
    if (msg.packets !== undefined) {
      ...
      for (let i = 0; i < resolved.packets.length; ++i) {
        resolved.packets[i] = VanjeePacket.Resolve(msg.packets[i]);
      }
    }
    else {
      resolved.packets = []
